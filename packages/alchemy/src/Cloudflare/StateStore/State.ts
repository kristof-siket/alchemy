    ...
    Effect.gen(function* () {
      ...
      const init = Effect.gen(function* () {
        if (yield* hasLocalStack(localStage)) {
          ...
          return yield* (yield* CliKit.CliKit).task(
            {
        ...
        const ensureLatest = ({
          ...
          Effect.gen(function* () {
            ...
            if (autoUpdateStateStore) {
              ...
              const shouldDeploy = yield* (yield* CliKit.CliKit).confirm({
                message:
        ...
        if (yield* isStateStoreServing(accountId)) {
          ...
          return yield* (yield* CliKit.CliKit)
            .confirm({
              message:
  ...
  Effect.gen(function* () {
    ...
    yield* workers.deleteScript({ accountId, scriptName, force: true }).pipe(
      ...
      Effect.catchTag("WorkerNotFound", () =>
        prompt.info(`  Worker '${scriptName}' not found (already gone).`),
      ),
    );
...
export const loginWithCloudflare = (profileName: string, force: boolean) =>
  Effect.gen(function* () {
    ...
    if (!isCI) yield* cli.info(`  url:     ${credentials.url}`);
    return credentials;
  }).pipe(
